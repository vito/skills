...
Parse YAML strings and access values by key.

### Global Function

| Function | Signature | Description |
|----------|-----------|-------------|
| `parseYAML` | `(data: String!, frontmatter: Boolean! = false): YAML!` | Parse a YAML string. When `frontmatter: true`, extracts YAML from between `---` markers. |

### YAML Methods

| Method | Signature | Description |
|--------|-----------|-------------|
| `string` | `(key: String!): String` | Get string value by key (nullable) |
| `int` | `(key: String!): Int` | Get integer value by key (nullable) |
| `bool` | `(key: String!): Boolean` | Get boolean value by key (nullable) |
| `yaml` | `(key: String!): YAML` | Get nested YAML mapping by key (nullable) |
| `list` | `(key: String!): [YAML!]` | Get list of YAML values by key (nullable) |
| `keys` | `(): [String!]!` | All top-level keys |
| `has` | `(key: String!): Boolean!` | Check if key exists |
| `asString` | `(): String` | Scalar value as string (nullable, null for mappings/lists) |
| `body` | `(): String!` | Content after frontmatter (only populated with `frontmatter: true`) |

### Example: Parsing Frontmatter

```dang
let doc = "---\nname: my-skill\ndescription: Does things\n---\n# Body content"

let meta = parseYAML(doc, frontmatter: true)
let name = meta.string("name") ?? ""
let description = meta.string("description") ?? ""
let body = meta.body
```
